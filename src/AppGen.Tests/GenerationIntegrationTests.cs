...
public class GenerationIntegrationTests
{
    ...
    public async Task Generate_sqlserver_writes_create_and_seed_scripts()
    {
        ...
        try
        {
            ...
            Assert.Contains("[Product_Id] BIGINT IDENTITY(1,1) NOT NULL", create);

            var seed = await File.ReadAllTextAsync(seedPath);
            Assert.Contains("SET IDENTITY_INSERT", seed);
        }
        finally
        {
            if (Directory.Exists(tempRoot))
    ...
    public async Task Generate_postgresql_writes_create_and_seed_scripts()
    {
        ...
        try
        {
            ...
            Assert.Contains("\"Product_Id\" BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL", create);
        }
        finally
        {
            if (Directory.Exists(tempRoot))
    ...
    [Fact]
    public async Task Generate_oracle_writes_identity_columns()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), "AppGenTests", Guid.NewGuid().ToString("N"));
        var outputDir = Path.Combine(tempRoot, "OracleScriptApp");

        try
        {
            var spec = SpecLoader.CreateDefault("OracleScriptApp", null, DatabaseProvider.Oracle);
            var product = new EntitySpec
            {
                Name = "Product",
                Properties =
            ...
            await new EntityGenerator(renderer).GenerateAsync(loaded, product, outputDir);
            loaded = await SpecLoader.LoadAsync(outputDir);
            await OracleScriptGenerator.WriteAsync(loaded, outputDir);

            var createPath = Path.Combine(outputDir, "scripts/oracle/001-create-tables.sql");
            Assert.True(File.Exists(createPath));

            var create = await File.ReadAllTextAsync(createPath);
            Assert.Contains("PRODUCT_ID NUMBER(19) GENERATED BY DEFAULT AS IDENTITY NOT NULL", create);

            var configurationPath = Path.Combine(outputDir, "src/OracleScriptApp.Persistence/Configurations/ProductConfiguration.cs");
            var configurationContent = await File.ReadAllTextAsync(configurationPath);
            Assert.Contains("UseIdentityColumn()", configurationContent);
        }
        finally
